...
if ( ! function_exists( 'understrap_posted_on' ) ) {
	...
	function understrap_posted_on() {
		...
		$posted_on   = apply_filters(
			...
			sprintf(
				'<span class="posted-on horizontal-line">%1$s <a href="%2$s" rel="bookmark" class="disabled text-decoration-none btn-link">%3$s</a></span>',
				esc_html_x( 'on', 'post date', 'understrap' ),
		...
		$byline      = apply_filters(
			...
			sprintf(
				'<span class="byline"> %1$s<span class="author vcard"> <a class="url fn n btn-link text-decoration-none" href="%2$s">%3$s</a></span></span>',
	...
	function understrap_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			*/
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links"><span class="btn btn-primary btn-sm mr-3"><i class="fa fa-tag mr-1"></i>Tags</span>' . esc_html__( '%s', 'understrap' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
		...
		edit_post_link(
			sprintf(
				...
				esc_html__( 'Edit Post %s', 'understrap' ),
